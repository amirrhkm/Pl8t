        ...
        @foreach ($months as $date)
            ...
            <a href="{{ route('staff.shift', ['staff' => $staff->id, 'year' => $date->year, 'month' => $date->month]) }}" 
               class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 text-center">{{ $date->format('F Y') }}</h5>
                <p class="font-normal text-gray-700 text-center">Total Hours: {{ number_format($totalHours, 1) }}</p>
                <p class="font-normal text-gray-700 text-center">Salary: RM {{ number_format($totalSalary, 2) }}</p>
            </a>
        @endforeach
    </div>
</x-layout>