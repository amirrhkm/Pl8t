<x-layout>
    ...
    <x-slot name="description">Edit Wastage Record</x-slot>
    ...
    <div class="w-1/2 bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4 mx-auto">
        ...
        @endif
        <form action="{{ route('wastages.update', $wastage) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="item" type="text" name="item" value="{{ $wastage->item }}" required>
            </div>
            <div class="mb-4 flex">
                <div class="w-1/2 pr-2">
                    ...
                    <input class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="quantity" type="number" name="quantity" value="{{ $wastage->quantity }}">
                </div>
                <div class="w-1/2 pl-2">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="weight">
                        Weight (kg)
                    </label>
                    <input class="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="weight" type="number" step="0.001" name="weight" value="{{ $wastage->weight }}">
                </div>
            </div>
            <div class="mb-4">
